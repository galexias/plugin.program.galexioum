...
# --- Offline builds source (userdata) ---
def _ensure_userdata_builds_file():
    try:
        import xbmcvfs
        import xbmc
        userdata_dir = xbmcvfs.translatePath('special://home/userdata/galexioum/')
        builds_file = xbmcvfs.translatePath('special://home/userdata/galexioum/builds.xml')

        if not xbmcvfs.exists(userdata_dir):
            xbmcvfs.mkdirs(userdata_dir)

        if not xbmcvfs.exists(builds_file):
            template = '''<?xml version="1.0" encoding="UTF-8"?>
<builds>
    <!--
      Βάλε εδώ τα builds σου (offline).
      Παράδειγμα:
      <build name="My Build" version="1.0" url="file:///storage/emulated/0/Download/mybuild.zip" />
    -->
</builds>
'''
            f = xbmcvfs.File(builds_file, 'w')
            f.write(template)
            f.close()
    except Exception:
        pass

def _silent_repo_updates():
    try:
        import xbmc
...
_ensure_userdata_builds_file()
_silent_repo_updates()
