...
def set_backup_location():
    """Let user pick a backup root folder and update settings.
    We store the chosen folder in the add-on setting 'path'.
    Actual build zips are stored under <path>/My_Builds
    """
    dialog = xbmcgui.Dialog()
    # Pick a folder (root). We'll create /My_Builds inside it.
    current_root = CONFIG.BACKUPLOCATION if hasattr(CONFIG, 'BACKUPLOCATION') else ''
    new_root = dialog.browse(3,
                            '[COLOR {0}]Select Backup Folder[/COLOR]'.format(CONFIG.COLOR2),
                            'files',
                            defaultt=current_root)
    if not new_root:
        logging.log_notify(CONFIG.ADDONTITLE,
                           '[COLOR {0}]Backup Folder: Cancelled[/COLOR]'.format(CONFIG.COLOR2))
        return

    # Normalize path (Kodi can return special:// paths or real paths)
    try:
        new_root = xbmcvfs.translatePath(new_root)
    except Exception:
        pass
    ...
    my_builds = os.path.join(new_root, 'My_Builds')
    try:
        xbmcvfs.mkdirs(my_builds)
    except Exception:
        pass

    # Persist
    CONFIG.set_setting('path', new_root)

    # Update in-memory paths for this session (so menu/backup uses it immediately)
    CONFIG.BACKUPLOCATION = new_root
    CONFIG.MYBUILDS = my_builds

    logging.log_notify(CONFIG.ADDONTITLE,
                       '[COLOR {0}]Backup Folder Set:[/COLOR] [COLOR {1}]{2}[/COLOR]'.format(
                           CONFIG.COLOR2, CONFIG.COLOR1, my_builds))
