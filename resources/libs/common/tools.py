...
def open_url(url, stream=False, check=False, cred=None, count=0):
    ...
    # Local/offline support: allow special:// and filesystem paths
    try:
        from urllib.parse import urlparse
        parsed = urlparse(url)
        is_web = bool(parsed.scheme and parsed.netloc)
    except Exception:
        is_web = False

    if not is_web:
        try:
            path = xbmcvfs.translatePath(url) if url.startswith('special://') else url
            if check:
            ...
            # xbmcvfs can return bytes or str depending on Kodi/python build
