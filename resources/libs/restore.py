...
class Restore:
    ...
    def choose(self, location):
        ...
        # For Full Build restores, allow choosing a custom destination folder (Omega-style).
        if location == CONFIG.HOME and not self.external:
            if not self.dialog.yesno(CONFIG.ADDONTITLE,
                                    '[COLOR {0}]Restore to the default Kodi folder?[/COLOR]'.format(CONFIG.COLOR2),
                                    '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, CONFIG.HOME),
                                    yeslabel='[B][COLOR springgreen]Default[/COLOR][/B]',
                                    nolabel='[B][COLOR yellow]Choose Folder[/COLOR][/B]'):
                new_loc = self.dialog.browse(3,
                                            '[COLOR {0}]Select destination folder for restore[/COLOR]'.format(CONFIG.COLOR2),
                                            'files',
                                            defaultt=CONFIG.HOME)
                if new_loc:
                    try:
                        new_loc = xbmcvfs.translatePath(new_loc)
                    except Exception:
                        pass
                    location = new_loc
